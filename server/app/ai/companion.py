...
def decide_companion_intent(session: Any, companion_id: str = "partner") -> dict:
    ...
    if floor_event:
        ...
        if current_floor != player_floor:
            ...
            if not has_reason_to_stay:
                ...
                traversal = floor_event.get("traversal")
                ...
                if (
                    ...
                    from app.game.map_slots import VERTICAL_MAP_CONTRACT as _map_contract
                    path = _map_contract["paths"].get(path_id, {})
                    authored = path.get("down") or path.get("out") or []
                    if authored:
    ...
    if (
        ...
        if session.vertical_round.phase == VerticalRoundPhase.ROOFTOP_INTRO:
            ...
            sx, _, sz = signal_slot.get(
                "aiApproachPosition",
                signal_slot.get("approachPosition", signal_slot["position"]),
            )
            return {
                "state": "EXPLORE_ZONE",
